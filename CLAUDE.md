...
> This file is automatically read by Claude AI to understand the project before any session.
...
- **Imports:** `google_fonts`, `animate_do` — always use these
...
- [x] `.gitignore`, `README.md` with screenshot & badges

## 🔲 What's Next (Planned)

- [ ] Firebase Authentication (login + signup for real)
...
1. **Do NOT change the color palette** — it matches the client's reference design
2. **Do NOT use `MaterialPageRoute`** — always use `PageRouteBuilder` with slide transition
...
7. Test on Android (primary target) — iOS secondary
